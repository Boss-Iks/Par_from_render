...
int main(int argc, char * argv[]) {
  ...
  CLIArgs cli = parse_cli(args, "render-aos");
  ...
  // implementation of the AOS rendering (filling memory with ray color)
  std::vector<Pixel> framebuffer;
  trace_rays_aos(cam, scene, framebuffer);
  writePPM_AOS(cli.output_path, framebuffer, cam.image_width, cam.image_height);
  return 0;
}
